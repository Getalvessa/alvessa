...
export default async function ForCleanersPage({ params }: Props) {
  const { locale } = await params;
...
function BenefitsSection() {
  ...
  const benefits = [
    ...
    { icon: Banknote, titleKey: 'benefit2Title' as const, descKey: 'benefit2Desc' as const },
...
function RequirementsSection() {
  const t = useTranslations('forCleaners');
  const reqs = ['req1', 'req2', 'req3', 'req4'] as const;
...
function CtaSection() {
  ...
  return (
    <div className="rounded-xl bg-foreground p-8 text-center">
      ...
      <Link
        href="/voor-schoonmakers/aanmelden"
