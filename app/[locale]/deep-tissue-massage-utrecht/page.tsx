...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'seoDeepTissue' });
  return buildMetadata({
    locale,
    path: 'deep-tissue-massage-utrecht',
    title: t('metaTitle'),
    description: t('metaDescription'),
  });
}

export default async function DeepTissueMassageUtrechtPage({ params }: Props) {
  const { locale } = await params;
...
function PageContent() {
  const t = useTranslations('seoDeepTissue');
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
      ...
      {/* Note */}
      <div className="mt-10 flex gap-3 rounded-xl border border-border bg-muted/30 p-4">
        <AlertCircle className="mt-0.5 h-5 w-5 shrink-0 text-muted-foreground" />
        <div>
          <p className="text-sm font-semibold text-foreground">{t('noteTitle')}</p>
          <p className="mt-1 text-sm text-muted-foreground">{t('noteBody')}</p>
        </div>
      </div>
