...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'seoHotelMassage' });
  return buildMetadata({
    locale,
    path: 'hotel-massage-utrecht',
    title: t('metaTitle'),
    description: t('metaDescription'),
  });
}

export default async function HotelMassageUtrechtPage({ params }: Props) {
  const { locale } = await params;
...
function PageContent() {
  const t = useTranslations('seoHotelMassage');

  const howSteps = ['how1', 'how2', 'how3'] as const;
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
      ...
      <section className="mt-14">
        <h2 className="mb-5 text-xl font-semibold text-foreground">{t('howTitle')}</h2>
      ...
      <section className="mt-12 rounded-xl border border-border p-5">
        ...
        <p className="text-sm text-muted-foreground">{t('hotelsBody')}</p>
      </section>
